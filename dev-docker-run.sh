...
# This script creates a development Docker container for api-recipe
...
docker stop karned-api-recipe || true
docker rm karned-api-recipe || true

# Run the container with local directory mounted
docker run -d \
  --name karned-api-recipe \
  ...
  -e API_NAME=api-recipe \
  -e API_TAG_NAME=recipes \
  killiankopp/api-recipe:1.1.1 \
...
echo "Access the API at http://localhost:9005"
echo "To view logs: docker logs karned-api-recipe"
echo "To stop: docker stop karned-api-recipe"
